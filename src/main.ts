...
import { chain_to_rpc, protocol_to_addresses, SILOV1_MARKET_ADDRESS, SILOV1_TOKEN_ADDRESS, VENUS_TOKEN_ADDRESS, VENUS_vTOKEN_ADDRESS } from "./config/config";
...
const main = async () => {
    ...
    //! thêm silov1
    let protocols = ['aavev3', 'venus', 'radiant', 'silov1'];
    let isolated: any = {
        'aavev3': {
                                    ...
                                    try {
                                        ...
                                        if (!isolated[protocol1][chain].includes(symbol1) &&
                                            ...
                                            if (APR_D_X_1 > 0 && APR_B_X_2 > 0 && APR_B_Y_1 > 0 && APR_D_Y_2 > 0 &&
                                                ...
                                                console.log(`Arbitrage ${tmpProtocol1}-${tmpProtocol2}, chain: ${chain} --> Market1: ${tmpProtocol1}, token 1: ${symbol1}, market2: ${tmpProtocol2}, token2: ${symbol2}, estimate apr: ${-LTV_X_1 * APR_B_Y_1 + LTV_X_1 * APR_D_Y_2 - LTV_X_1 * LTV_Y_2 * APR_B_X_2 + LTV_X_1 * LTV_Y_2 * APR_D_X_1}`);
                                                let data: any = {
                                                    chain: chain,
    ...
    // for (let chain in chains) {
    //     for (let token1Address in Object.values(VENUS_vTOKEN_ADDRESS))
    //         for (let token2Address in Object.values(VENUS_vTOKEN_ADDRESS)) {
    //             let token = new Token(chain);
    //             let token1Venus = new Venus(token1Address, chain);
    //             let token2Venus = new Venus(token2Address, chain);
    //             const APR_D_X = Number(await token1Venus.getSupplyAPR());
    //             const APR_B_Y = Number(await token2Venus.getBorrowAPR());
    //             const LTV_X = Number(await token1Venus.getLtv());

    //             let symbol1 = await token.getSymbol(token1Address);
    //             symbol1 = (symbol1 as string).slice(1, symbol1.length);

    //             let symbol2 = await token.getSymbol(token2Address);
    //             symbol2 = (symbol2 as string).slice(1, symbol2.length);

    //             if (APR_B_Y > 0 && APR_D_X > 0 && APR_D_X > APR_B_Y && LTV_X > 0) {
    //                 console.log(`VENUS --> Supply token: ${symbol1} and deposit token: ${symbol2}`);
    //                 let data: any = {
    //                     pairToken: [symbol1, symbol2],
    //                     apr: {
    //                         [symbol1]: {
    ...
    //                     }
    //                 }
    //                 if (!result.loop.venus) {
    //                     result.loop.venus = [];
    //                 }
    //                 result.loop.venus.push(data);
    //             }
    //             await sleep(1000);
    //         }
    // }
    // console.log("Done venus!");
